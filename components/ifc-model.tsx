...
export function IFCModel({ modelData, outlineLayer }: IFCModelProps) {
  ...
  const fetchPropertiesForSelectedElement = useCallback(async () => {
    ...
    try {
      const props = await getElementProperties(
        ifcApi,
        internalApiIdForEffects,
        selectedElement.expressID,
      );
      ...
      console.error(
        `IFCModel (${modelData.id}): Error fetching properties for element ${selectedElement.expressID}:`,
        error,
      );
      setElementProperties({ error: "Failed to fetch properties" });
    }
  }, [ifcApi, selectedElement, internalApiIdForEffects, modelData.id, setElementProperties]);
